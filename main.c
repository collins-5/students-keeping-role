...
//confirming the validity of the school email
    ...
    scanf("%f", &s[n].previous_marks);

    s[n].avg=totalmarks/c;
    if((s[n].avg)>(s[n].previous_marks)){
...
void display_students(struct student s[], int n) {
    printf("Name\t  student email. \t    Roll No.\tprevious_marks\t current_marks\n");
    for (int i = 0; i < n; i++) {
        printf("%s\t  %s\t %d\t     %.2f\t\t  %.2f\n", s[i].name,s[i].email, s[i].rollno, s[i].previous_marks,s[i].avg);
    }
}
...
int main() {
    ...
    while (1) { //the user is supposed to choose an operation among the given ones. continuously until he/she decides to exit the program
        ...
        switch (choice) {
            case 1:
                add_student(s, n);
                n++; // increment number of students4321
                break;
            case 2:
                printf("******************\n");
